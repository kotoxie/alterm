...
export function TabBar({
  ...
  // Drag state
  const [draggingId, setDraggingId] = useState<string | null>(null);
  const [insertBefore, setInsertBefore] = useState<string | null>(null); // id of tab to insert before, or 'END'
  ...
  function handleDragStart(e: React.DragEvent, id: string) {
    ...
    // Custom ghost: clone the tab element, apply styles
    const el = e.currentTarget as HTMLElement;
    const ghost = el.cloneNode(true) as HTMLElement;
    ghost.style.cssText = `
      position: fixed; top: -9999px; left: -9999px;
  ...
  function handleDragOver(e: React.DragEvent, id: string) {
    ...
    setInsertBefore(isLeftHalf ? id : null);

    // If right half, we insert after this tab — track the next tab's id
    if (!isLeftHalf) {
      const idx = tabs.findIndex((t) => t.id === id);
      const next = tabs[idx + 1];
      setInsertBefore(next ? next.id : 'END');
  ...
  function handleDragOverBar(e: React.DragEvent) {
    // Hovering the bar past all tabs → insert at end
    e.preventDefault();
    if (draggingId) setInsertBefore('END');
  }
  ...
  function handleDrop(e: React.DragEvent) {
    ...
    const reordered = currentIds.filter((id) => id !== draggingId);
    let toIdx: number;
    if (insertBefore === 'END' || insertBefore === null) {
      toIdx = reordered.length;
    } else {
      ...
      <div className="flex items-center h-9 bg-surface-alt border-b border-border shrink-0">
        ...
        >
          {tabs.map((tab) => {
            ...
            const showIndicator = insertBefore === tab.id && draggingId !== tab.id;

            return (
              <div key={tab.id} className="relative flex items-center h-full shrink-0">
                {/* Drop insertion indicator — vertical accent line on left */}
                {showIndicator && (
                  <div className="absolute left-0 top-1 bottom-1 w-0.5 bg-accent rounded-full z-10 shadow-[0_0_6px_rgba(124,58,237,0.8)]" />
                )}
                ...
                <div
                  ...
                  onDragEnd={handleDragEnd}
                  className={clsx(
                    'flex items-center gap-2 px-3 h-full text-sm cursor-grab active:cursor-grabbing border-r border-border select-none transition-opacity duration-150',
          ...
          {insertBefore === 'END' && draggingId && (
            <div className="relative h-full flex items-center px-1">
              <div className="w-0.5 h-6 bg-accent rounded-full shadow-[0_0_6px_rgba(124,58,237,0.8)]" />
            </div>
          )}
        </div>
